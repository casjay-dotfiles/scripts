...
__gen_config() {
  ...
  [[ "$INIT_CONFIG" = "TRUE" ]] || printf_green "$CHEAT_SH_CONFIG_DIR/$CHEAT_SH_CONFIG_FILE"
...
CHEAT_SH_ERROR_MESSAGE="${CHEAT_SH_ERROR_MESSAGE:-Well something seems to have gone wrong}"
...
__am_i_online --error "$CHEAT_SH_URL" || exit 1 # exit 1 if no internet
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
