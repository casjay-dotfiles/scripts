...
__gen_config() {
  ...
  printf_green "$TERMBIN_COM_CONFIG_DIR/$TERMBIN_COM_CONFIG_FILE"
...
EOF
  if [ -f "$TERMBIN_COM_CONFIG_DIR/$TERMBIN_COM_CONFIG_FILE" ]; then
    printf_green "Your config file for termbin.com has been created"
    true
  else
...
TERMBIN_COM_TEMP_FILE="${TERMBIN_COM_TEMP_FILE:-$TERMBIN_COM_TEMP_DIR/$(mktemp XXXXXX)}"
