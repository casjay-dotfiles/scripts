...
_apimgr_completion() {
  ...
  #####################################################################
  ARRAY="github gitlab gitea forgejo codeberg gitee pagure sourcehut bitbucket docker ghcr harbor quay verify user org repo issue pr release tag image api"
  ARRAY+=""
  #####################################################################
  ...
  if [[ ${cur} == --no* ]]; then
    ...
    --limit)
      ...
      return 0
      ;;
    github|gitlab|gitea|forgejo|codeberg|gitee|pagure|sourcehut|sr.ht|srht|bitbucket|docker|ghcr|harbor|quay)
