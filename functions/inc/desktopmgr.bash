...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
git_repo_urls() {
  REPO="${REPO:-https://github.com/desktopmgr}"
...
user_installdirs() {
  ...
  if [[ $(id -u) -eq 0 ]] || [[ $EUID -eq 0 ]] || [[ "$WHOAMI" = "root" ]]; then
    ...
    USRUPDATEDIR="$SHARE/CasjaysDev/apps/${SCRIPTS_PREFIX:-desktopmgr}"
    SYSUPDATEDIR="$SYSSHARE/CasjaysDev/apps/${SCRIPTS_PREFIX:-desktopmgr}"
  else
    INSTALL_TYPE=user
    HOME="${HOME:-/tmp/$USER}"
    BIN="$HOME/.local/bin"
...
system_installdirs() {
  SCRIPTS_PREFIX="${SCRIPTS_PREFIX:-desktopmgr}"
  APPNAME="${APPNAME:-installer}"
  REPORAW="${REPORAW:-}"
  if [[ $(id -u) -eq 0 ]] || [[ $EUID -eq 0 ]] || [[ "$WHOAMI" = "root" ]]; then
    if [[ $(uname -s) =~ Darwin ]]; then HOME="/usr/local/home/root"; fi
    ...
    USRUPDATEDIR="/usr/local/share/CasjaysDev/apps/${SCRIPTS_PREFIX:-desktopmgr}"
    SYSUPDATEDIR="/usr/local/share/CasjaysDev/apps/${SCRIPTS_PREFIX:-desktopmgr}"
  else
    INSTALL_TYPE=system
    ...
    WALLPAPERS="$HOME/.local/share/wallpapers"
    USRUPDATEDIR="$HOME/.local/share/CasjaysDev/apps/${SCRIPTS_PREFIX:-desktopmgr}"
    SYSUPDATEDIR="$HOME/.local/share/CasjaysDev/apps/${SCRIPTS_PREFIX:-desktopmgr}"
  fi
  APPDIR="${APPDIR:-$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/$APPNAME}"
  INSTDIR="${INSTDIR:-$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/$APPNAME}"
