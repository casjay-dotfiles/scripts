...
localVersion="${localVersion:-202207031902-git}"
requiredVersion="${requiredVersion:-202207031902-git}"
...
###################### setup folders - user ######################
user_installdirs() {
  [ "$APPNAME" = "$SCRIPTS_PREFIX" ] &&
    APPDIR="${APPDIR//$APPNAME\/$SCRIPTS_PREFIX/$APPNAME}"
  [ "$APPNAME" = "$SCRIPTS_PREFIX" ] &&
    INSTDIR="${INSTDIR//$APPNAME\/$SCRIPTS_PREFIX/$APPNAME}"
  ...
  if [[ $(id -u) -eq 0 ]] || [[ $EUID -eq 0 ]] || [[ "$WHOAMI" = "root" ]]; then
    ...
    USRUPDATEDIR="$SHARE/CasjaysDev/apps/${SCRIPTS_PREFIX:-devenvmgr}"
    SYSUPDATEDIR="$SYSSHARE/CasjaysDev/apps/${SCRIPTS_PREFIX:-devenvmgr}"
  else
    INSTALL_TYPE=user
    HOME="${HOME:-/tmp/$USER}"
    BIN="$HOME/.local/bin"
...
system_installdirs() {
  SCRIPTS_PREFIX="${SCRIPTS_PREFIX:-devenvmgr}"
  ...
  if [[ $(id -u) -eq 0 ]] || [[ $EUID -eq 0 ]] || [[ "$WHOAMI" = "root" ]]; then
    ...
    USRUPDATEDIR="/usr/local/share/CasjaysDev/apps/${SCRIPTS_PREFIX:-devenvmgr}"
    SYSUPDATEDIR="/usr/local/share/CasjaysDev/apps/${SCRIPTS_PREFIX:-devenvmgr}"
  else
    INSTALL_TYPE=system
    ...
    WALLPAPERS="$HOME/.local/share/wallpapers"
    USRUPDATEDIR="$HOME/.local/share/CasjaysDev/apps/${SCRIPTS_PREFIX:-devenvmgr}"
    SYSUPDATEDIR="$HOME/.local/share/CasjaysDev/apps/${SCRIPTS_PREFIX:-devenvmgr}"
  fi
  APPDIR="${APPDIR:-$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/$APPNAME}"
  INSTDIR="${INSTDIR:-$HOME/.local/share/CasjaysDev/$SCRIPTS_PREFIX/$APPNAME}"
...
run_install_list() {
  if [ -d "$USRUPDATEDIR" ] && [ -n "$(ls -A "$USRUPDATEDIR/$1" 2>/dev/null)" ]; then
    ...
    declare -a LSINST="$(ls "$USRUPDATEDIR/" 2>/dev/null || echo '')"
    if [ -z "${LSINST[0]}" ]; then
