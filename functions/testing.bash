...
sudo() { PATH=$PATH builtin command sudo "$@"; }
...
###################### Apps ######################
#vim "file"
vim="$(type -P /usr/local/bin/vim 2>/dev/null || type -P vim 2>/dev/null)"
__vim() { $vim "$@"; }
#mkd dir
__mkd() {
  for d in "$@"; do
...
# find replace
__find_replace() { find -L "$3" -type f -exec "$sed" -i 's#'$1'#'$2'#g' {} \; 2>/dev/null; }
#replace file
...
__api_test() {
  local message="${*:-}"
  if am_i_online && builtin type -P jq &>/dev/null; then
    return 0
  else
...
# if builtin type -P  zenity &>/dev/null && [ -n "$DISPLAY" ] && [ -z "$SSH_TTY" ]; then
#   __execute() {
#     local CMD="$1"
#     local MSG="$2"
...
__if_os_id() {
  ...
  elif builtin type -P sw_vers &>/dev/null; then
