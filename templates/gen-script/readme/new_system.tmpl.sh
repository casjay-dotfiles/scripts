...
$([[ -f "$path_filename" ]] && for o in $(grep -s 'ARRAY=' $path_filename 2>/dev/null | grep -E ',| ' | head -n1 | sed 's|ARRAY=||g;s|,| |g;s|"||g' | tr ' ' 'n'); do echo '__printf_help "4" "'$(__filename $GEN_SCRIPT_NEWFILE 2>/dev/null)' '$o'  -  "'; done)
