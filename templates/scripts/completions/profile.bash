...
_GEN_SCRIPT_REPLACE_FILENAME_completion() {
  ...
  ___grep_env() { GREP_COLORS="" grep -sE '^.*=*..*$' "$1" 2>/dev/null | grep -v '^#' | grep "${2:-^}" | sed 's|"||g' 2>/dev/null | grep '^' || false; }
  #####################################################################
  ...
  CONFDIR="$HOME/.config/misc/settings/GEN_SCRIPT_REPLACE_FILENAME"
  SEARCHDIR="${CONFDIR:-$HOME/.config/misc/settings/GEN_SCRIPT_REPLACE_FILENAME}"
  #####################################################################
  SHOW_COMP_OPTS=""
  #####################################################################
  SHORTOPTS=""
