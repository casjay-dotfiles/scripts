...
__initialize_replace_variables() {
  [ $# -ne 0 ] || return 1
  [ -n "$RUN_DIR" ] && __find_replace "REPLACE_RUN_DIR" "${RUN_DIR:-/run/$SERVICE_NAME}" "$1" || echo "RUN_DIR is not set"
  [ -n "$LOG_DIR" ] && __find_replace "REPLACE_LOG_DIR" "${LOG_DIR:-/data/log/$SERVICE_NAME}" "$1" || echo "LOG_DIR is not set"
  [ -n "$ETC_DIR" ] && __find_replace "REPLACE_ETC_DIR" "${ETC_DIR:-/etc/$SERVICE_NAME}" "$1" || echo "ETC_DIR is not set"
  [ -n "$DATA_DIR" ] && __find_replace "REPLACE_DATA_DIR" "${DATA_DIR:-/data/$SERVICE_NAME}" "$1" || echo "DATA_DIR is not set"
  [ -n "$CONF_DIR" ] && __find_replace "REPLACE_CONFIG_DIR" "${CONF_DIR:-/config/$SERVICE_NAME}" "$1" || echo "CONF_DIR is not set"
  [ -n "$SERVICE_PORT" ] && __find_replace "REPLACE_SERVER_PORT" "${SERVICE_PORT:-80}" "$1" || echo "SERVICE_PORT is not set"
  [ -n "$HOSTNAME" ] && __find_replace "REPLACE_SERVER_NAME" "${FULL_DOMAIN_NAME:-$HOSTNAME}" "$1" || echo "HOSTNAME is not set"
  [ -n "$CONTAINER_NAME" ] && __find_replace "REPLACE_SERVER_SOFTWARE" "${CONTAINER_NAME:-docker}" "$1" || echo "CONTAINER_NAME is not set"
  [ -n "$WWW_ROOT_DIR" ] && __find_replace "REPLACE_SERVER_WWW_DIR" "${WWW_ROOT_DIR:-/usr/share/httpd/default}" "$1"
  [ -n "$SERVER_ADMIN" ] && __find_replace "REPLACE_SERVER_ADMIN" "${SERVER_ADMIN:-root@${EMAIL_DOMAIN:-${FULL_DOMAIN_NAME:-$HOSTNAME}}}" "$1"
