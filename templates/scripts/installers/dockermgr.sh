...
REPO="${DOCKERMGRREPO:-https://github.com/$SCRIPTS_PREFIX}/GEN_SCRIPT_REPLACE_APPNAME"
...
__printf_spacing() { printf "%-${2:-30}b%b\n" "$(tput setaf "${1:-5}" 2>/dev/null)${3}" "${4}$(tput sgr0 2>/dev/null)"; }
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if [ "$CONTAINER_INSTALLED" = "true" ] || __docker_ps_all -q; then
  ...
  if [ "$HOSTS_WRITABLE" = "true" ]; then
      ...
      for vhost in $NGINX_VHOST_NAMES; do
        if ! grep -sq " $vhost$" "/etc/hosts"; then
          if echo "$vhost" | grep -qFv '*'; then
            __printf_spacing "44" "40" "Adding to /etc/hosts:" "$vhost $CONTAINER_WEB_SERVER_LISTEN_ON"
            __printf_file_space "40" "$CONTAINER_WEB_SERVER_LISTEN_ON" "$vhost" | sudo tee -a "/etc/hosts" &>/dev/null
          fi
        fi
      done
    ...
    if [ -n "$HOST_NGINX_INTERNAL_DOMAIN" ]; then
      if ! grep -sq " $HOST_NGINX_INTERNAL_DOMAIN$" "/etc/hosts"; then
        __printf_spacing "44" "40" "Adding to /etc/hosts:" "$HOST_NGINX_INTERNAL_DOMAIN $HOST_LISTEN_ADDR"
        __printf_file_space "40" "$HOST_LISTEN_ADDR" "$HOST_NGINX_INTERNAL_DOMAIN" | sudo tee -a "/etc/hosts" &>/dev/null
      fi
    fi
    if ! grep -sq " $CONTAINER_HOSTNAME$" "/etc/hosts"; then
      __printf_spacing "44" "40" "Adding to /etc/hosts:" "$CONTAINER_HOSTNAME $HOST_LISTEN_ADDR"
      __printf_file_space "40" "$HOST_LISTEN_ADDR" "$CONTAINER_HOSTNAME" | sudo tee -a "/etc/hosts" &>/dev/null
    fi
    show_hosts_message_banner="true"
    [ "$show_hosts_message_banner" = "true" ] && printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
    unset show_hosts_message_banner
  fi
  __printf_spacing "3" "40" "The container name is:" "$CONTAINER_NAME"
  __printf_spacing "3" "40" "Containers data is saved in:" "$DATADIR"
  __printf_spacing "3" "40" "The container is listening on:" "$HOST_LISTEN_ADDR"
  __printf_spacing "3" "40" "The domain name is set to:" "$CONTAINER_DOMAINNAME"
  __printf_spacing "3" "40" "The hostname name is set to:" "$CONTAINER_HOSTNAME"
  if [ -n "$HOST_NGINX_INTERNAL_DOMAIN" ]; then
    __printf_spacing "3" "40" "The internal name is set to:" "$HOST_NGINX_INTERNAL_DOMAIN"
  fi
  printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  if [ "$HOST_CRON_ENABLED" = "yes" ] && [ -n "$HOST_CRON_COMMAND" ] && [ -n "$NGINX_PROXY_URL" ]; then
    ...
    __printf_spacing "6" "40" "Setting cron user to:" "$HOST_CRON_USER"
    __printf_spacing "6" "40" "Setting schedule to:" "$HOST_CRON_SCHEDULE"
    __printf_spacing "3" "40" "Saving cron job to: /etc/cron.d/$CONTAINER_NAME"
  ...
  if __ssl_certs; then
    ...
    if __sudo_exec cp -Rf "$HOST_SSL_CA" "$CONTAINER_SSL_CA"; then
      ...
      __printf_spacing "3" "40" "Copied CA Cert to:" "$CONTAINER_SSL_CA"
    fi
    if __sudo_exec cp -Rf "$HOST_SSL_CRT" "$CONTAINER_SSL_CRT"; then
      __sudo_exec chmod -Rf 666 "$CONTAINER_SSL_DIR"
      __printf_spacing "3" "40" "Copied certificate to:" "$CONTAINER_SSL_CRT"
    fi
    if __sudo_exec cp -Rf "$HOST_SSL_KEY" "$CONTAINER_SSL_KEY"; then
      __sudo_exec chmod -Rf 666 "$CONTAINER_SSL_DIR"
      __printf_spacing "3" "40" "Copied private key to:" "$CONTAINER_SSL_KEY"
    fi
    __sudo_exec chown -Rf "$USER":"$USER" "$CONTAINER_SSL_DIR" &>/dev/null
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  fi
  if [ "$DOCKER_CREATE_NET" ]; then
    __printf_spacing "5" "40" "Created docker network:" "$HOST_DOCKER_NETWORK"
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  fi
  if [ "$NGINX_IS_INSTALLED" = "yes" ]; then
    __printf_spacing "6" "40" "nginx vhost name:" "$CONTAINER_HOSTNAME"
    __printf_spacing "6" "40" "nginx website:" "$NGINX_PROXY_URL"
    if [ -f "$NGINX_CONF_FILE" ]; then
      __printf_spacing "6" "40" "nginx config file installed to:" "$NGINX_CONF_FILE"
    fi
    if [ -f "$NGINX_INC_CONFIG" ]; then
      __printf_spacing "6" "40" "nginx vhost file installed to:" "$NGINX_INC_CONFIG"
    fi
    if [ -f "$NGINX_VHOST_CONFIG" ]; then
      __printf_spacing "6" "40" "nginx custom vhost file installed to:" "$NGINX_VHOST_CONFIG"
    fi
    if [ -n "$NGINX_INTERNAL_IS_SET" ]; then
      __printf_spacing "6" "40" "nginx internal vhost file installed to:" "$NGINX_INTERNAL_IS_SET"
    fi
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  fi
  if [ -n "$SET_PORT" ] && [ -n "$NGINX_PROXY_URL" ]; then
    MESSAGE="true"
    __printf_spacing "33" "40" "Server address:" "$NGINX_PROXY_URL"
    if [ -n "$NGINX_VHOST_NAMES" ]; then
      NGINX_VHOST_NAMES="${NGINX_VHOST_NAMES//,/ }"
      for vhost in $NGINX_VHOST_NAMES; do
        __printf_spacing "33" "40" "vhost name:" "$vhost"
      done
    fi
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  fi
  if [ -n "$CONTAINER_USER_ADMIN_PASS_HASH" ]; then
    ...
    __printf_spacing "6" "40" "raw password is:" "$CONTAINER_USER_ADMIN_PASS_RAW"
    if [ "$CONTAINER_USER_ADMIN_PASS_RAW" != "$CONTAINER_USER_ADMIN_PASS_HASH" ]; then
      __printf_spacing "6" "40" "hashed password is:" "$CONTAINER_USER_ADMIN_PASS_HASH"
    fi
  fi
  if [ -n "$CONTAINER_USER_NAME" ]; then
    show_user_footer="true"
    __printf_spacing "6" "40" "Username is:" "$CONTAINER_USER_NAME"
  fi
  if [ -n "$CONTAINER_USER_PASS" ]; then
    show_user_footer="true"
    __printf_spacing "33" "40" "Password is:" "$CONTAINER_USER_PASS"
  fi
  if [ "$CONTAINER_DATABASE_USER_ROOT" ]; then
    show_user_footer="true"
    __printf_spacing "33" "40" "Database root user:" "$CONTAINER_DATABASE_USER_ROOT"
  fi
  if [ "$CONTAINER_DATABASE_PASS_ROOT" ]; then
    show_user_footer="true"
    __printf_spacing "33" "40" "Database root password:" "$CONTAINER_DATABASE_PASS_ROOT"
  fi
  if [ "$CONTAINER_DATABASE_USER_NORMAL" ]; then
    show_user_footer="true"
    __printf_spacing "33" "40" "Database user:" "$CONTAINER_DATABASE_USER_NORMAL"
  fi
  if [ "$CONTAINER_DATABASE_PASS_NORMAL" ]; then
    show_user_footer="true"
    __printf_spacing "33" "40" "Database password:" "$CONTAINER_DATABASE_PASS_NORMAL"
  fi
  [ "$show_user_footer" = "true" ] && printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  if [ "$SHOW_DATABASE_INFO" = "true" ]; then
    __printf_spacing "3" "40" "Database is running on:" "$CONTAINER_DATABASE_PROTO"
    if [ -n "$MESSAGE_CONTAINER_DATABASE" ]; then
      __printf_spacing "6" "40" "$MESSAGE_CONTAINER_DATABASE"
    fi
    if [ -n "$MESSAGE_COUCHDB" ]; then
      __printf_spacing "6" "40" "Database files are saved to:" "$DATABASE_DIR_COUCHDB"
    fi
    if [ -n "$MESSAGE_SQLITE" ]; then
      __printf_spacing "6" "40" "Database files are saved to:" "$DATABASE_DIR_SQLITE"
    fi
    if [ -n "$MESSAGE_MARIADB" ]; then
      __printf_spacing "6" "40" "Database files are saved to:" "$DATABASE_DIR_MARIADB"
    fi
    if [ -n "$MESSAGE_MONGODB" ]; then
      __printf_spacing "6" "40" "Database files are saved to:" "$DATABASE_DIR_MONGODB"
    fi
    if [ -n "$MESSAGE_PGSQL" ]; then
      __printf_spacing "6" "40" "Database files are saved to:" "$DATABASE_DIR_POSTGRES"
    fi
    if [ -n "$MESSAGE_REDIS" ]; then
      __printf_spacing "6" "40" "Database files are saved to:" "$DATABASE_DIR_REDIS"
    fi
    if [ -n "$MESSAGE_SUPABASE" ]; then
      __printf_spacing "6" "40" "Database files are saved to:" "$DATABASE_DIR_SUPABASE"
    fi
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  fi
  if [ -f "$DATADIR/config/auth/htpasswd" ]; then
    MESSAGE="true"
    __printf_spacing "5" "40" "Username:" "root"
    __printf_spacing "5" "40" "Password:" "${SET_USER_PASS:-toor}"
    __printf_spacing "5" "40" "htpasswd File:" "/config/auth/htpasswd"
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  fi
  if [ -z "$SET_PORT" ]; then
    __printf_spacing "3" "40" "This container does not have services configured"
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  else
    for service in $SET_PORT; do
      if [ "$service" != "--publish" ] && [ "$service" != " " ] && [ -n "$service" ]; then
        ...
        if [ -n "$listen" ]; then
          __printf_spacing "6" "40" "Port $set_service is mapped to:" "$set_listen"
        fi
      fi
  ...
  if [ -f "$DOCKERMGR_INSTALL_SCRIPT" ]; then
    __printf_spacing "3" "40" "Script saved to:" "$DOCKERMGR_INSTALL_SCRIPT"
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  fi
  if [ -f "$DOCKERMGR_CONFIG_DIR/env/$APPNAME.env.conf" ] || [ -f "$DOCKERMGR_CONFIG_DIR/env/$APPNAME.custom.conf" ]; then
    if [ -f "$DOCKERMGR_CONFIG_DIR/env/$APPNAME.env.conf" ]; then
      __printf_spacing "2" "40" "variables saved to:" "$DOCKERMGR_CONFIG_DIR/env/$APPNAME.env.conf"
    fi
    if [ -f "$DOCKERMGR_CONFIG_DIR/env/$APPNAME.custom.conf" ]; then
      __printf_spacing "2" "40" "Container variables saved to:" "$DOCKERMGR_CONFIG_DIR/env/$APPNAME.custom.conf"
    fi
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  fi
  if [ -n "$CONTAINER_DEFAULT_USERNAME" ] || [ -n "$CONTAINER_DEFAULT_PASSWORD" ]; then
    [ -n "$CONTAINER_DEFAULT_USERNAME" ] && __printf_spacing "6" "40" "Containers default username is:" "$CONTAINER_DEFAULT_USERNAME"
    [ -n "$CONTAINER_DEFAULT_PASSWORD" ] && __printf_spacing "6" "40" "Containers default password is:" "$CONTAINER_DEFAULT_PASSWORD"
    printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n'
  fi
  ...
  __printf_spacing "6" "40" "$APPNAME has been installed to:" "$install_dir"
  printf '# - - - - - - - - - - - - - - - - - - - - - - - - - -\n\n'
  __show_post_message
else
  __printf_color "6" "The container $CONTAINER_NAME seems to have failed"
  if [ "$ERROR_LOG" = "true" ]; then
    __printf_spacing "3" "40" "Errors logged to:" "${TMP:-/tmp}/$APPNAME.err.log"
  else
    printf_red "Something seems to have gone wrong with the install"
  fi
  if [ -f "$DOCKERMGR_INSTALL_SCRIPT" ]; then
    __printf_spacing "3" "40" "Script:" "$DOCKERMGR_INSTALL_SCRIPT"
  fi
  exit 10
fi
