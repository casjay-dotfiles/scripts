...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
__system_service_enable() { \systemctl enable --now "$1" || return 1; }
__system_service_disable() { \systemctl disable --now "$1" || return 1; }
__system_service_start() { \systemctl restart "$1" || \systemctl start "$1" || return 1; }
