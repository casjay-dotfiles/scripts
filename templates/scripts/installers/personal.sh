  ...
  if [[ "$OSTYPE" =~ ^linux ]]; then
    if ! cmd_exists systemmgr; then
      if (sudo -vn && sudo -ln) 2>&1 | grep -v 'may not' >/dev/null; then
        sudo bash -c "$(curl -LSs https://github.com/systemmgr/installer/raw/GEN_SCRIPTS_REPLACE_DEFAULT_BRANCH/install.sh)"
        sudo bash -c "$(curl -LSs https://github.com/systemmgr/installer/raw/GEN_SCRIPTS_REPLACE_DEFAULT_BRANCH/install.sh)"
      else
        printf_red '\t\tplease run sudo bash -c "$(curl -LSs https://github.com/systemmgr/installer/raw/GEN_SCRIPTS_REPLACE_DEFAULT_BRANCH/install.sh)\n"'
        exit 1
      fi
    fi
  fi
